...
elseif strcmp(Subname, 'Mori')
	subname='Mori';
elseif strcmp(Subname, 'Sakamoto_upper')
	subname='Sakamoto';
elseif strcmp(Subname, 'Sugino_upper')
	subname='Sugino';
elseif strcmp(Subname, 'Mori_upper')
	subname='Mori';
elseif strcmp(Subname, 'Sakamoto_lower')
	subname='Sakamoto';
elseif strcmp(Subname, 'Sugino_lower')
	subname='Sugino';
elseif strcmp(Subname, 'Mori_lower')
	subname='Mori';
end
...
%% Display
% head = load(fullfile(Subname,'tess_outer_skin_surface.mat')).head;
% innerskull = load(fullfile(Subname,'tess_inner_skull_surface.mat')).innerskull;
% outerskull = load(fullfile(Subname,'tess_outer_skull_surface.mat')).outerskull;
cortex = load(fullfile(subname, 'cortex_vertices_whole_brain_15002.mat')).cortex;

%% bold大きい順に並び替え、必要な個数だけを取り出す（センサ位置最適化、メイン発火部位選択用、それ以外の時はコメントアウト）
if main==1
	want = 100;
	tmp = [bold, pos_bs];
...
%% センサ位置最適化、メイン＋ノイズ選択用、それ以外の時はコメントアウト）
elseif main==2
